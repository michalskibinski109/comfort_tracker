...
class Model:
    ...
    the field will be set to the default value. (For now works only for the fields that are set in the init method.)
    ...
    Args:
        ...
        `bad_habits` (int): Everyone can define their own bad habits. It could be smoking, drinking,porn, etc.
    ...
    methods:
        ...
        `__str__`: Returns a beauty representation of the model.
    """
    ...
    def __set_initial_values(self) -> None:
        ...
        try:
            ...
            return
        if str(datetime.datetime.now().date()) in row["date"]:
    ...
    def __load_data(self, data_path: Path) -> pd.DataFrame:
        if not data_path.exists():
            ...
            self._logger.info(f"Created file with header {df.columns}")
        else:
